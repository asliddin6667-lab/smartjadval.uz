...
import { swapActive, swapTeacherHours } from "../utils/swapGroups";
  ...
  const requiredHoursFor = useCallback((tid) => {
    ...
    classes.forEach((cls) => {
      (classSubjects[cls.id] || []).forEach((a) => {
        ...
        // Almashinuv (2 fan bitta vaqtda) — har bir ustoz band. Ustoz
        // blokning IKKALA soatida ham tursa `wh × 2`, faqat bittasida
        // tursa `wh` soat ([swapGroups.js](../utils/swapGroups.js)).
        if (swapActive(a)) {
          total += swapTeacherHours(a, wh).get(tid) || 0;
          return;
        }
